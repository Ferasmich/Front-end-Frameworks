...
## Earlier JavaScript exercises

Branch `01` now contains both the movie app and completed exercises 1–9.
The exercises are in `exercises/`, with their original tests in `tests/`.
Movie app tests are in `src/test/`. Run `npm run test:run` to check both sets.
